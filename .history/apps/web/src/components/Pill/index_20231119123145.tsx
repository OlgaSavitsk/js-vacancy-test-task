...
const Pill: FC<PillProps> = ({ value, onRemove }) => {
  ...
  const paymentRange = (paymentFrom && `$${paymentFrom}`);
  const payment = (paymentTo && `-$${paymentTo}`);
  // const payment = typeof value === 'string' ? value : paymentRange;
  console.log(paymentFrom, paymentRange);
  return (
    <Badge
      color="gray"
      size="xl"
    ...
    >
      {typeof value === 'string' ? value : paymentRange && payment}
