...
const Pill: FC<PillProps> = ({ value, onRemove }) => {
  const paymentRange = typeof value === {} && ((value!.payment_from && `$${value!.payment_from}`)(value!.payment_to && `-$${value!.payment_to}`));
  const payment = typeof value === 'string' ? value : ((value!.payment_from && `$${value!.payment_from}`)(value!.payment_to && `-$${value!.payment_to}`));
  console.log(payment);
  return (
    <Badge
      color="gray"
