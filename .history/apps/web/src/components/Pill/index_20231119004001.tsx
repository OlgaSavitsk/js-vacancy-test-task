...
const Pill: FC<PillProps> = ({ value, onRemove }) => {
  ...
  const paymentRange = value!.price.price) && ((value.price.payment_from && `$${value.price.payment_from}`)(value.price.payment_to && `-$${value.payment_to}`));
  const payment = typeof value === 'string' ? value : '';
  console.log(payment);
  return (
    <Badge
      color="gray"
