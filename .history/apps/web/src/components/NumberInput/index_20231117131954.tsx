...
interface NumberInput extends NumericFormatProps {
  ...
  setDebounced: (val: string) => void,
}

const NestedInput: FC<NumberInput> = memo(
  ({ register, control, name, suffix, placeholder, label, data, setDebounced }) => {
    const [debouncedPaymentTo] = useDebouncedState('', 1000);
        ...
        render={({ field: { onChange, value } }) => (
          <Input.Wrapper label={label}>
            <Input
              ...
              defaultValue={debouncedPaymentTo}
            />
          </Input.Wrapper>
        )}
      />
