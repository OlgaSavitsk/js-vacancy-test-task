...
const paymentValue = z.array(z.string().nullable());
...
const Home: NextPage = () => {
  ...
  const [filterDate, setFilterDate] = useState<PaymentRangeValue>([null]);
  ...
  const handleRemove = (val: string) => setFilterDate((current) => current.filter((v) => v !== val));
  ...
  const onSubmit = ({ payment_from, payment_to }: SalaryParams) => {
    setFilterDate([payment_from, payment_to]);
    if (!payment_from) {
      setParams((prev) => ({
        ...prev,
        filter: {},
          ...
          <Grid.Col span="auto">
            <Stack>
              ...
              >
                ...
                <Pill value={filterDate} onRemove={() => handleRemove('222')} />
              </Skeleton>
