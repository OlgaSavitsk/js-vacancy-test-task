...
const Home: NextPage = () => {
  ...
  const handleRemove = (val: number) => setFilterDate(
    (current) => current.filter((_, index) => index !== val),
  );
  ...
  const onSubmit = ({ payment_from, payment_to }: SalaryParams) => {
    ...
    setFilterDate([{
      ...
      payment_to: methods.getValues().payment_to!.replace(/\$/g, ''),
    }]);
  };
  ...
  useLayoutEffect(() => {
    ...
    debouncedSearch && setFilterDate((prev) => [...prev, debouncedSearch]);
