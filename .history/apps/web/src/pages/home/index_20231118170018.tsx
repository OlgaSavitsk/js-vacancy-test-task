...
const paymentValue = z.array(z.number().nullable().or(z.string().nullable()));
...
const Home: NextPage = () => {
  ...
  const handleSearch = useCallback((event: ChangeEvent<HTMLInputElement>) => {
    ...
    setFilterDate((prev) => [...prev, event.target.value]);
  }, []);

  const handleRemove = (val: string) => setFilterDate(
  ...
  const onSubmit = ({ payment_from, payment_to }: SalaryParams) => {
    ...
    if (payment_to) {
      ...
      setFilterDate([+payment_from!.replace(/\$/g, ''), +payment_to!.replace(/\$/g, '')]);
    }
  };
              ...
              <Group position="apart">
                <Stack>
                  ...
                  >
                    ...
                    {filterDate.map<Pill value={filterDate} onRemove={() => handleRemove('222')} />}
                  </Skeleton>
                </Stack>
                <Select
                  size="md"
