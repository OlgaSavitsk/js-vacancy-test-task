...
const Home: NextPage = () => {
  ...
  const [debouncedFilter] = useDebouncedValue(value.payment_to, 1000);
  ...
  useLayoutEffect(() => {
    setParams((prev) => ({
      ...
      price: {
        ...
        payment_to: debouncedFilter!,
      },
      perPage: PER_PAGE,
    }));
    setFilterDate([params.price!, debouncedSearch!]);
  }, [debouncedFilter, debouncedSearch, params.price, params.searchValue, value]);

  const { isDirty, dirtyFields } = methods.formState;

  console.log('filterDate', isDirty);
