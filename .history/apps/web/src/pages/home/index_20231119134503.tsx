...
// const paymentValue = z.tuple([
//   z.array(z.string().nullable()),
//   z.string().optional()]);

// const paymentValue = z.array(
//   z.array(z.string().nullable()).optional()
//     .and(z.string().optional()),
// );

const paymentValue = z.union(
  [z.array(z.string().nullable()).optional(), (z.string().optional())],
);
...
const Home: NextPage = () => {
  ...
  // const [debouncedFilter] = useDebouncedState(value.payment_to, 1000);
  ...
  const onSubmit = ({ payment_from, payment_to }: SalaryParams) => {
    setParams((prev) => ({
      ...prev,
      price: [payment_from ? payment_from.replace(/\$/g, '') : null, payment_to ? payment_to.replace(/\$/g, '') : null],
    }));
  };
  ...
  useLayoutEffect(() => {
    ...
    // setFilterDate([params.price
    //   ? [params.price.payment_from, params.price.payment_to]
    //   : undefined, debouncedSearch]);
  ...
  const handleRemove = useCallback((val: number) => {
    setFilterDate(
      filterDate!.filter((_, index) => index !== val),
    );
    // setParams((prev) => ({
    //   ...prev,
    //   price: undefined,
  ...
  console.log('filterDate', filterDate, methods.formState.defaultValues);
              ...
              <Group position="apart">
                <Stack>
                  ...
                  >
                    ...
                    (params.price || params.searchValue) && (
                    <Pill
                      value={params.price || params.searchValue}
