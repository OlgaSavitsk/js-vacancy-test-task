...
const paymentValue = z.array(schema.or(z.string().nullable()));
...
const Home: NextPage = () => {
  ...
  const handleRemove = (val: PaymentRangeValue) => setFilterDate(
    (current:) => {
      if(val) {
        current.filter((v: (string | { payment_from: string | null; payment_to: string | null; } | null)[]) => v !== val)
      }
      Object.values(current).filter((v) => v !== val),
    }
  );
  ...
  useLayoutEffect(() => {
    ...
    setFilterDate((prev) => [...prev, debouncedSearch]);
  }, [debouncedSearch]);
