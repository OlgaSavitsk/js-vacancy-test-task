...
const Home: NextPage = () => {
  ...
  }, [debouncedSearch, param.price, params.searchValue]);

  const { data, isLoading: isListLoading } = productsApi.useList(params);

  const filterData = useMemo(() => setFilterDate([params.price!, params.searchValue!]), [params]);
  // eslint-disable-next-line max-len
  const handleRemove = useCallback((val: number) => setFilterDate(filterData.filter((_, index) => index !== val)), [filterData]);
