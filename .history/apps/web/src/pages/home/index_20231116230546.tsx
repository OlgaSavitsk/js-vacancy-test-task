...
const Home: NextPage = () => {
  ...
  const [debouncedPaymentTo] = useDebouncedValue(methods.getValues().payment_to, 1000, { leading: true });
  ...
  const onSubmit = (data: SalaryParams) => {
    ...
    if (data.payment_to && debouncedPaymentTo) {
      setParams((prev) => ({
        ...prev,
        filter: {
          price: {
            payment_from: data.payment_from.replace(/\$/g, ''),
            payment_to: debouncedPaymentTo ? data.payment_to.replace(/\$/g, '') : '',
          },
        },
      }));
    }
