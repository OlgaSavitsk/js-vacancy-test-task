...
async function handler(ctx: AppKoaContext<ValidatedData>) {
  ...
  if (!user.cart.length) {
    ...
    const productInCart = await userService.find({ _id: user._id, cart: { $elemMatch: { _id: product._id } } });
    if (productInCart) {
      const index = _.findIndex(productInCart.cart, ['_id', product._id]);
      userService.increaseQuantity(user._id, product);
    } else {
      await userService.updateMany(
        { _id: user._id },
        ({ cart }) => ({
