...
const increaseQuantity = (_id: string, productId: string) => {
  return service.atomic.updateMany(
    { _id: _id, cart: { $elemMatch: { _id: productId } } },
    { $inc: { 'cart.$.quantity':  1 }, $set: { updatedOn: new Date() } },
    {},
    { arrayFilters: [{'quantity': { 'gte': 1 } }] },
  );
};
