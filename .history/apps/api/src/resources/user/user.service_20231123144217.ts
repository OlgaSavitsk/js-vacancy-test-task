...
const increaseQuantity = (_id: string, product: Products) => {
  return service.atomic.updateMany(
    ...
    { $inc: { 'cart.$[el].quantity':  1 }, $set: { updatedOn: new Date(), 'cart.$[el].quantity': product.quantity + 1 } },
    {},
  ...
  //     cart: [...cart, { ...(_.find(cart, ['_id', product._id])!),
  //       _id: product._id, 
  //       quantity: _.find(cart, ['_id', product._id])!.quantity! + 1 }],
  //   }),
  // );
};
