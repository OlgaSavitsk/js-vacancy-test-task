...
const updateCart = (_id: string, productId: string) => {
  return service.atomic.updateMany(
    { _id }, 
    {
      ...
      $set: { 'cart.$."Error parsing array filter :: caused by :: The top-level field name must be an alphanumeric string beginning with a lowercase letter, found '_id'':new Date() },
    },
    {},
    // { arrayFilters: [ { _id: { $eq: productId } } ] },
  );
};
